...
class ApplesoftBasic(Transformer):
    ...
    def _clear_text(self):
        ...
        # テキストのクリア
        pyxel.rect(self._text_clip_x * self._font_size_x, self._text_clip_y * self._font_size_y, self._text_clip_size_x * self._font_size_x, self._text_clip_size_y * self._font_size_y, self._color_back)
    ...
    def _clear_text_line(self):
        ...
        # テキストの消去
        pyxel.rect(self._cursor_x * self._font_size_x, self._cursor_y * self._font_size_y, (self._text_clip_x + self._text_clip_size_x - self._cursor_x) * self._font_size_x, self._font_size_y, self._color_back)
    ...
    def _putc(self, c, cursor = True, flush = False):
        ...
        # １文字の出力
        pyxel.blt(x, y, 0, ((ord(c) - 0x20) & 0x0f) * 0x08 + color, ((ord(c) - 0x20) >> 4) * 0x08, self._font_size_x, self._font_size_y)
    ...
    def _newline(self):
        ...
        if self._cursor_y >= self._text_clip_y + self._text_clip_size_y:
            pyxel.image(4, system = True).copy(self._text_clip_x * self._font_size_x, self._text_clip_y * self._font_size_y, 4, self._text_clip_x * self._font_size_x, (self._text_clip_y + 1) * self._font_size_y, self._text_clip_size_x * self._font_size_x, (self._text_clip_size_y - 1) * self._font_size_y)
            pyxel.rect(self._text_clip_x * self._font_size_x, (self._text_clip_y + self._text_clip_size_y - 1) * self._font_size_y, self._text_clip_size_x * self._font_size_x, self._font_size_y, self._color_back)
            pyxel.flip()
    ...
    def _clear_hires(self):
        ...
        if self._screen_mode == 'hires':
            pyxel.rect(self._hires_clip_x, self._hires_clip_y, self._hires_clip_size_x, self._hires_clip_size_y, self._color_back)
    ...
    def _plot(self, positions):
        ...
        # 点
        if len(positions) == 2:
            pyxel.pset(positions[0] * self._hires_rate_x, positions[1] * self._hires_rate_y, self._color_hires)
        ...
        # 線
            ...
            while i <= len(positions) - 4:
                ...
                pyxel.line(x_0, y_0, x_1, y_1, color)
                i = i + 2
    ...
    def _view_image(self, basename):
        ...
        if os.path.isfile(path):
            ...
            # イメージの読み込み
            pyxel.image(4, system = True).load(0, 0, path)
...
#       exit()
    ...
    # Pyxel の初期化
    pyxel.init(240, 192, caption = 'Akalabeth: World of Doom')
